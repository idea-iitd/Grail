#solve_nbm1 -> uses NBM-1 hop, solve -> uses NBM-2 hop. 
import importlib
import ast
import copy
import numpy as np
...
def process_chunk(chunk, function_names, module_name, n):
    ...
    for idx, (g_pair, nf, gt) in chunk:
        ...
        def compute_costs_for_n(g1, g2, labels1, labels2, weights):
            costs = []
            for func in funcs:
                # change solve function here: solve_nbm1 or solve, both give similar results but nbm1 is faster for denser datasets
                nodemap = solve_nbm1(g1, g2, labels1,labels2,func)
                # save mapping if required
                cost = 0
                y = 0
...
def evaluate(testset, funcset, layer_val, num_processes=96):
    """Parallelized RMSE computation for graph edit distances with multiprocessing."""
    module_name = f'discovered_programs.submod_{funcset}' 
    ...
    with open(f'./top_15/funcs_{funcset}.txt', 'r') as file:
    ...
    with open(f"{save_path}results_testset_{testset}_funcset_{funcset}_.txt","w") as f:
        for i in geds:
            f.write(f"{i}\n")
...
if __name__ == '__main__':
    ...
    evaluate(args.testset, args.func_set, layer_val, num_processes=os.cpu_count())