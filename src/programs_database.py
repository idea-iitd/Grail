...
def _reduce_score(scores_per_test: ScoresPerTest) -> float:
  ...
  scores_array = np.array(list(scores_per_test.values()))
  average_score = np.mean(scores_array) if scores_array.size > 0 else 0
  print("Average score from _reduce_score function: ", scores_array, average_score)
...
def _get_signature(scores_per_test: ScoresPerTest) -> Signature:
  ...
  # change made to handle multiple graphs
...
class ProgramsDatabase:
  ...
  def _register_program_in_island(
    ...
    # print("Registering program in database:", program)
    # print("in island_id:",island_id)
    self._islands[island_id].register_program(program, scores_per_test)
    score = _reduce_score(scores_per_test)
    if score >= self._best_score_per_island[island_id]:
      self._best_program_per_island[island_id] = program
      # print("Best program in island:", self._best_program_per_island[island_id])
  ...
  def reset_submod(self) -> None:
    for island_id in range(len(self._islands)):
      self._islands[island_id] = Island(
          ...
          self._config.cluster_sampling_temperature_period)
      self._best_score_per_island[island_id] = -float('inf')
     #* new best k programs are registered in solver
...
class Island:
  ...
  def get_prompt(self) -> tuple[str, int]:
    ...
    # print("Sorted imple:", sorted_implementations)
  ...
  def _generate_prompt(
    ...
    for i, implementation in enumerate(implementations):
      ...
      # print("Versioned function before manipulation:", implementation)
      # print("Versioned function :", code_manipulation.text_to_function(implementation))
