    ...
    for idx, (g_pair, nf) in chunk:
        ...
        for i, func_name in enumerate(function_names):
            ...
            try:
                ...
                func_flag[i] = False
                print(f'Error flag set for {func_name} for graph pair {idx}')
            ...
            if pr is not None:
                for _ in range(n):
                    try:
                        ...
                        print(f'Error flag set for {func_name} for graph pair {idx} (2)')
                        func_flag[i] = False
                        break
                    if x is None:
                        break
        ...
        def compute_costs_for_n(g1, g2, labels1, labels2, weights):
            ...
            for func in funcs:
                if(func == 100000):
                    cost = 100000 # high number to indicate error and so that this function is not picked in min
                else:
        ...
        row = [idx,'-'] + compute_costs_for_n(g1, g2, labels1, labels2, weights)
        results.append(row)

    return results

def evaluate(layer_val, valset, func_set, num_processes):
    # import os
    # print(os.cpu_count())
    """Parallelized RMSE computation for graph edit distances with multiprocessing."""
    module_name = f'top_fns_transfer.{func_set}'
    
    with open(f'../data/val/{valset}_val_1k.txt', 'r') as file:
        content = file.read()

        g_pairs_str = content.split("g_pairs: ")[1].split("node_features: ")[0].strip()
        nf_str = content.split("node_features: ")[1].strip()
    ...
    with open(f'./val_transfer/functions_{func_set}.txt', 'r') as file:
        function_names = [line.strip() for line in file.readlines()]

    global func_flag 
    func_flag = [True] * len(function_names)

    headers = ['Graph Pair Number', 'Ground Truth'] + [f"{name}" for name in function_names]
    ws.append(headers)
    print("validating")
    start_time=time.time()

    # Split graph pairs into chunks for parallel processing
    data = list(enumerate(zip(g_pairs, nfs), start=1))
    ...
    wb.save(f'./val_transfer/val_set_{valset}_funcs_{func_set}_{n}.xlsx')
...
if __name__ == '__main__':
    ...
    evaluate(layer_val, args.valset, args.func_set, num_processes=96)